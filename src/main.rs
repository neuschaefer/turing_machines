...
impl TMDesc {
    ...
    fn parse_line(line: &str) -> Result<Vec<&str>, ()> {
        if line.starts_with("#") {
    ...
    fn handle_line(&mut self, words: &[&str]) {
...
impl<'a> TM<'a> {
    fn new<'a>(desc: &'a TMDesc, input: &str) -> TM<'a> {
        TM {
            desc: desc,
            head: 0,
    ...
    fn get_tape_output(&self) -> String {
        let mut s = String::new();
...
fn main() {
    ...
    let input_file = File::open(&Path::new("../input.txt"))
